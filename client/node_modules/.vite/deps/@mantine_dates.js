...
var Month = factory((_props, ref) => {
  ...
  const rows = dates.map((row, rowIndex) => {
    const cells = row.map((date, cellIndex) => {
      ...
      const ariaLabel = (getDayAriaLabel == null ? void 0 : getDayAriaLabel(date)) || (0, import_dayjs9.default)(date).locale(locale || ctx.locale).format("D MMMM YYYY");
...
function isDateValid({ date, maxDate, minDate }) {
  ...
  if (maxDate && (0, import_dayjs31.default)(date).isAfter(maxDate, "date")) {
    return false;
  }
  if (minDate && (0, import_dayjs31.default)(date).isBefore(minDate, "date")) {
    return false;
  }
  return true;
}
...
var DateInput = factory((_props, ref) => {
  ...
  const defaultDateParser = (val) => {
    const parsedDate = (0, import_dayjs32.default)(val, valueFormat, ctx.getLocale(locale)).toDate();
  ...
  const formatValue2 = (val) => val ? (0, import_dayjs32.default)(val).locale(ctx.getLocale(locale)).format(valueFormat) : "";
  ...
  const _getDayProps = (day) => ({
    ...
    selected: (0, import_dayjs32.default)(_value).isSame(day, "day"),
    onClick: () => {
      const valueWithTime = preserveTime ? assignTime(_value, day) : day;
      const val = clearable && _allowDeselect ? (0, import_dayjs32.default)(_value).isSame(day, "day") ? null : valueWithTime : valueWithTime;
      setValue(val);
...
var DateTimePicker = factory((_props, ref) => {
  ...
  const formatTime = (dateValue) => dateValue ? (0, import_dayjs33.default)(dateValue).format(withSeconds ? "HH:mm:ss" : "HH:mm") : "";
  ...
  const formattedValue = _value ? (0, import_dayjs33.default)(_value).locale(ctx.getLocale(locale)).format(_valueFormat) : "";
...
function getDefaultClampedDate({ minDate, maxDate, timezone }) {
  ...
  if (minDate && (0, import_dayjs34.default)(today).isBefore(minDate)) {
    return minDate;
  }
  if (maxDate && (0, import_dayjs34.default)(today).isAfter(maxDate)) {
    return maxDate;
  }
  return today;
}
...
function getFormattedDate({
  ...
  const formatDate = (value) => (0, import_dayjs35.default)(value).locale(locale).format(format);
  if (type === "default") {
